...
for (i in seq_len(nrow(samples))) {
  if (dir.exists(samples$dir[i])) {
    ...
    modality2_name <- strsplit(names(raw)[2], " ")[[1]][1]
    ...
    # create an assay object using the first word of the matrix name
    x[[modality2_name]] <- CreateAssayObject(counts = raw[[2]])
