...
integrate <- function(data) {
  immune.anchors <- FindIntegrationAnchors(object.list = data, dims = 1:13)
  immune.integrated <- IntegrateData(anchorset = immune.anchors, dims = 1:13)
  DefaultAssay(immune.integrated) <- "integrated"
  all.genes <- rownames(immune.integrated)
  immune.integrated <- ScaleData(immune.integrated, verbose = FALSE, features = all.genes)
  immune.integrated <- RunPCA(immune.integrated, npcs = 13, verbose = FALSE)
  immune.integrated <- RunUMAP(immune.integrated, reduction = "pca", dims = 1:13)
  immune.integrated <- FindNeighbors(immune.integrated, reduction = "pca", dims = 1:13)
...
seurat_object <- function(file_name, p.dir, s, g) {
  ...
  object <- subset(object, subset = nCount_RNA > 500 & nCount_RNA < 60000)
